...
A 2D pool game built from scratch in Python and Pygame — a custom physics engine, an AI opponent, and a full menu system, built solo over eight stages from a bouncing-ball prototype to final polish.
...
![Customise Game screen — each player enters a name and picks a colour off a rainbow slider, which then colours their turn indicator and ball tracker in-game](../../images/projects/billiard-club/customise-game.png)
...
I built it in stages — physics and collisions first, then the table and UI, then cue mechanics, rules, AI, menus, audio, and a final polish pass — testing after each one. The trickiest parts got prototyped in isolation first: a standalone `ballPhysics.py` for collisions and restitution, and a `sliders.py` for the shot-power UI, before merging both into `main.py`.
