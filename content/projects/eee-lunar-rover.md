...
A WiFi-controlled lunar surface rover, built over a summer as ELEC40006 Group 25 at Imperial College London — six of us (Aila Danish, Hywel Evans, Beiyan Shi, Charles Wu, Saahil Zaki, and me) designing, building, and testing a rover that drives across a mocked-up lunar arena, homes in on artificial "rocks," and reads their magnetic, infrared, ultrasound, and RF signatures to classify each one.
...
All four readings feed one truth-table lookup — each rock type is a unique combination of IR rate, ultrasound, and magnetic polarity — duplicated in the Flask bridge so the front end stays sensible even if the Arduino omits `rock_type`.
